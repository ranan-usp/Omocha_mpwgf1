...
def create_dacvia(cell,m_index,via_index,DBU,base_pos,first_l,last_l,mode = 0):
    ...
    # make metal box
    box_size = [0.4*DBU,1.2*DBU]
    print(box_size[0],box_size[1])
    
    for l in range(first_l,last_l+1):
      
      if mode == 0:
          middle_point = [base_pos[0] + box_size[0]//2,base_pos[1] + box_size[1]//2]
          pos1 = np.array(middle_point) - np.array(box_size)//2
          pos2 = np.array(middle_point) + np.array(box_size)//2
          create_Box2(cell,m_index[l-1],pos1,pos2)

          if l == last_l:
             break
          
          via_point = np.array(middle_point) - np.array([0,box_size[1]//4])
          pos1 = np.array(via_point) - np.array([via_size,via_size])//2
          pos2 = np.array(via_point) + np.array([via_size,via_size])//2
          create_Box2(cell,via_index[l-1],pos1,pos2)
          ...
          middle_point = [base_pos[0] + box_size[1]//2,base_pos[1] + box_size[0]//2]
          pos1 = np.array(middle_point) - np.array([box_size[1],box_size[0]])//2
          pos2 = np.array(middle_point) + np.array([box_size[1],box_size[0]])//2
          create_Box2(cell,m_index[l-1],pos1,pos2)

          if l == last_l:
             break

          via_point = np.array(middle_point) - np.array([box_size[1]//4,0])
          pos1 = np.array(via_point) - np.array([via_size,via_size])//2
          pos2 = np.array(via_point) + np.array([via_size,via_size])//2
          create_Box2(cell,via_index[l-1],pos1,pos2)
...
if __name__ == '__main__':
    ...
    capacitor = layout.layer(LayerInfo.new(117,5))
    ...
    create_Box3(cell,cap_index,path,root_wire_width+subroot_wire_width*2-0.28*DBU)
    ...
    connect_met_width = 0.5*DBU
    connect_met_width2 = 0.5*DBU
    connect_set = set()
    ...
    for count in range(LOOP+dummy_met):
        ...
        if count > dummy_met//2-1 and count < LOOP+dummy_met//2:
          ...
          create_dacvia(cell,m_index,v_index,DBU,[current_pos_x-0.2*DBU,temp2_y-1.2*DBU],2,4)
          ...
          if connect_num not in connect_set:
            ...
            if connect_num != 'd':
              ...
              upper_pos = [path_ext[1][0]- 0.2*DBU, path_ext[1][1] - (BIT+1)*extend_wire]
